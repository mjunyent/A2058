#include "myGlow.h"
#include "Trinidad/Director/director.h"

myGlowScene::myGlowScene(int radius, float strength, int mix, TBO *glowMapL, TBO *glowMapR) {
	this->radius = radius;
	...
	filterGlow = new Shader("Shaders/Post/general.vert", "Shaders/Vladivostok/myGlowFilter.frag");
	...
	blur = new BlurScene(width, height, radius, strength);
...
void myGlowScene::draw(int s, double time) {
